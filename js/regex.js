...
function executaRegex(valores) {
	...
	while (resultado = objetoRegex.exec(textoTarget)) {
		...
		if(resultado[0] === "") {
			throw Error("Regex returned a null value.");
		}

		console.log("Match: " + resultado[0]);
...
function pegaValoresDoForm() {
	 ...
	 inputTarget 	= document.querySelector('#target');
	 inputPattern 	= document.querySelector('#pattern')
	inputPattern.focus();

	 checkboxIndex 	= document.querySelector('#mostraIndex');
	 checkboxGroups 	= document.querySelector('#mostraGrupos');
...
function _verifiqueInputs(inputTarget, inputPattern) {
	if(!inputTarget.value) {
		inputTarget.placeholder = 'Type the target';
	}

	if(!inputPattern.value) {
		inputPattern.placeholder = 'Type the pattern';
	}
...
function limparResultados() {
	console.clear();
	document.querySelector('#labelResultados').innerHTML = '0 Matches';
	document.querySelector('#resultado').value = '';
	document.querySelector('#resultado').placeholder = '0 Matches';
	document.querySelector("#highlightText").innerHTML = '<em>0 Matches</em>';
